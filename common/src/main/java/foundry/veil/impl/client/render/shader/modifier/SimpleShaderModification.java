...
public class SimpleShaderModification implements ShaderModification {
  ...
  public SimpleShaderModification(int version, int priority,
  ...
  public void inject(GlslTree tree, VeilJobParameters parameters)
    ...
    //            "\n";
    //        }
    //        tree.parseAndInjectNodes(parser,
    //        ASTInjectionPoint.BEFORE_DECLARATIONS, includes);
    //
    //        if (!StringUtil.isNullOrEmpty(this.uniform)) {
    //            tree.parseAndInjectNodes(parser,
    //            ASTInjectionPoint.BEFORE_DECLARATIONS,
    //            this.fillPlaceholders(this.uniform).split("\n"));
    //        }
    //
    //        if (!StringUtil.isNullOrEmpty(this.output)) {
    //            tree.parseAndInjectNodes(parser,
    //            ASTInjectionPoint.BEFORE_DECLARATIONS,
    //            this.fillPlaceholders(this.output).split("\n"));
    //        }
    //
    //        Root root = tree.getRoot();
    //        for (Function function : this.functions) {
    //            String name = function.name();
    //            ChildNodeList<Statement> statements =
    //            root.identifierIndex.getStream(name)
    //                    .map(id ->
    //                    id.getBranchAncestor(FunctionDefinition.class,
    ...
    //                        }
    //                        return
    //                        definition.getFunctionPrototype().getParameters().size()
    //                        == paramCount;
    //                    })
    //                    .findFirst()
    ...
    //                        int paramCount = function.parameters();
    //                        if (paramCount == -1) {
    //                            return new IOException("Unknown function: " +
    //                            name);
    //                        }
    //                        return new IOException("Unknown function with " +
    //                        paramCount + " parameters: " + name);
    //                    }).getStatements();
    //
    //            Statement statement = parser.parseStatement(root,
    //            this.fillPlaceholders("{" + function.code() + "}")); if
    //            (function.head()) {
  ...
  protected String getPlaceholder(String key) { return key; }
