 ...
 * <p>Shaders can be recompiled using {@link #recompile(ResourceLocation,
 * ResourceProvider)} or
 * {@link #recompile(ResourceLocation, ResourceProvider, ShaderCompiler)} to use
 * a custom compiler.</p>
 *
 * @author Ocelot
 * @see ShaderCompiler
...
public class ShaderManager implements PreparableReloadListener, Closeable {
  ...
  private static final Gson GSON =
      new GsonBuilder()
          ...
          .registerTypeAdapter(ProgramDefinition.class,
                               new ProgramDefinition.Deserializer())
          .create();

  public static final FileToIdConverter INCLUDE_LISTER =
      new FileToIdConverter("pinwheel/shaders/include", ".glsl");
  public static final ShaderSourceSet PROGRAM_SET =
      new ShaderSourceSet("pinwheel/shaders/program");
  public static final ShaderSourceSet DEFERRED_SET =
      new ShaderSourceSet("pinwheel/shaders/deferred");

  private static final Map<Integer, String> TYPES = Map.of(
      GL_VERTEX_SHADER, "vertex", GL_TESS_CONTROL_SHADER, "tesselation_control",
      GL_TESS_EVALUATION_SHADER, "tesselation_evaluation", GL_GEOMETRY_SHADER,
      "geometry", GL_FRAGMENT_SHADER, "fragment", GL_COMPUTE_SHADER, "compute");
   ...
   */
  public ShaderManager(ShaderSourceSet sourceSet,
                       ShaderPreDefinitions shaderPreDefinitions) {
  ...
  private void onDefinitionChanged(String definition) {
    this.shaders.values().forEach(shader -> {
      if (shader.getDefinitionDependencies().contains(definition)) {
        Veil.LOGGER.debug("{} changed, recompiling {}", definition,
                          shader.getId());
  ...
  private ProgramDefinition parseDefinition(ResourceLocation id,
                                            ResourceProvider provider)
      throws IOException {
    try (Reader reader = provider.openAsReader(
             this.sourceSet.getShaderDefinitionLister().idToFile(id))) {
      ProgramDefinition definition =
          GsonHelper.fromJson(GSON, reader, ProgramDefinition.class);
      if (definition.vertex() == null &&
          definition.tesselationControl() == null &&
          definition.tesselationEvaluation() == null &&
          definition.geometry() == null && definition.fragment() == null &&
          definition.compute() == null) {
        throw new JsonSyntaxException(
            "Shader programs must define at least 1 shader type");
      }
  ...
  private void readShader(ResourceManager resourceManager,
                          Map<ResourceLocation, ProgramDefinition> definitions,
    ...
    try {
      ...
      if (definitions.put(id, definition) != null) {
        throw new IllegalStateException("Duplicate shader ignored with ID " +
                                        id);
      }

      for (Int2ObjectMap.Entry<ProgramDefinition.ShaderSource> shader :
           definition.shaders().int2ObjectEntrySet()) {
        FileToIdConverter typeConverter =
            this.sourceSet.getTypeConverter(shader.getIntKey());
        ResourceLocation location =
            typeConverter.idToFile(shader.getValue().location());
        ...
        try (InputStream stream = resource.open()) {
          ...
          Resource fileResource = new Resource(
              resource.source(), () -> new ByteArrayInputStream(source));
          shaderSources.put(location, fileResource);
        } catch (Throwable t) {
          throw new IOException("Failed to load " +
                                    getTypeName(shader.getIntKey()) + " shader",
                                t);
        }
      }
    } catch (IOException | IllegalArgumentException | JsonParseException e) {
      Veil.LOGGER.error("Couldn't parse shader {} from {}", id,
                        this.sourceSet.getShaderDefinitionLister().idToFile(id),
                        e);
    }
  }

  private Map<ResourceLocation, Resource>
  readIncludes(ResourceManager resourceManager) {
    ...
    for (Map.Entry<ResourceLocation, Resource> entry :
      ...
      try {
        ...
        Veil.LOGGER.error("Couldn't parse shader import {} from {}", id,
                          location, e);
      }
    }
  ...
  private void compile(ShaderProgram program, ProgramDefinition definition,
                       ShaderCompiler compiler) {
    ResourceLocation id = program.getId();
    try {
      program.compile(new ShaderCompiler.Context(this.definitions,
                                                 this.sourceSet, definition),
                      compiler);
  ...
  public void recompile(ResourceLocation id, ResourceProvider provider) {
    try (ShaderCompiler compiler =
             this.addProcessors(ShaderCompiler.direct(provider), provider)) {
   ...
   * @param id       The id of the shader to recompile
   * @param provider The source of resources
   * @param compiler The compiler instance to use. If unsure, use {@link
   *     #recompile(ResourceLocation, ResourceProvider)}
   */
  public void recompile(ResourceLocation id, ResourceProvider provider,
                        ShaderCompiler compiler) {
   ...
   * @return The retrieved shader or <code>null</code> if there is no valid
   *     shader with that id
   */
   ...
   */
  public ShaderSourceSet getSourceSet() { return this.sourceSet; }

  private ReloadState prepare(ResourceManager resourceManager,
                              Collection<ResourceLocation> shaders) {
  ...
  private void apply(ShaderManager.ReloadState reloadState) {
    ...
    ResourceProvider sourceProvider =
        loc -> Optional.ofNullable(reloadState.shaderSources().get(loc));
    try (ShaderCompiler compiler = this.addProcessors(
             ShaderCompiler.cached(sourceProvider), sourceProvider)) {
      for (Map.Entry<ResourceLocation, ProgramDefinition> entry :
           reloadState.definitions().entrySet()) {
    ...
    Veil.LOGGER.info("Loaded {} shaders from: {}", this.shaders.size(),
                     this.sourceSet.getFolder());
  }

  private void applyRecompile(ShaderManager.ReloadState reloadState,
                              Collection<ResourceLocation> shaders) {
    ResourceProvider sourceProvider =
        loc -> Optional.ofNullable(reloadState.shaderSources().get(loc));
    try (ShaderCompiler compiler = this.addProcessors(
    ...
    Veil.LOGGER.info("Recompiled {} shaders from: {}", shaders.size(),
                     this.sourceSet.getFolder());
  }
  ...
  private void scheduleRecompile(int attempt) {
    ...
    client.tell(() -> {
      ...
      this.recompileFuture =
          CompletableFuture
              .supplyAsync(
                  ()
                      -> this.prepare(client.getResourceManager(), shaders),
                  Util.backgroundExecutor())
              .thenAcceptAsync(
                  state -> this.applyRecompile(state, shaders), client)
              .handle((value, e) -> {
                if (e != null) {
                ...
                if (attempt >= 3) {
                  Veil.LOGGER.error(
                      "Failed to recompile shaders after {} attempts", attempt);
                  return value;
                }
  ...
  public void scheduleRecompile(ResourceLocation shader) {
    synchronized (this.dirtyShaders) { this.dirtyShaders.add(shader); }
  ...
  reload(PreparationBarrier preparationBarrier, ResourceManager resourceManager,
                      ...
                      .supplyAsync(
                          ()
                            ...
                            FileToIdConverter lister =
                                this.sourceSet.getShaderDefinitionLister();
                            Set<ResourceLocation> shaderIds =
                                lister.listMatchingResources(resourceManager)
                                    .keySet()
                                    .stream()
                                    .map(lister::fileToId)
  ...
  public String getName() {
    return this.getClass().getSimpleName() + " " +
        this.getSourceSet().getFolder();
  }

  /**
   * Recompiles all shaders in the background.
   *
   * @param resourceManager    The manager for resources. Shader files and
   *     definitions are loaded from this manager
  ...
  public CompletableFuture<Void> reload(ResourceManager resourceManager,
    ...
    return this.reloadFuture = CompletableFuture.allOf(
               this.reload(this, resourceManager, backgroundExecutor,
                           gameExecutor),
               this.reload(framebufferManager, resourceManager,
                           backgroundExecutor, gameExecutor),
               this.reload(postProcessingManager, resourceManager,
                           backgroundExecutor, gameExecutor));
  }

  private CompletableFuture<Void> reload(PreparableReloadListener listener,
                                         ResourceManager resourceManager,
                                         Executor backgroundExecutor,
                                         Executor gameExecutor) {
    return listener.reload(CompletableFuture::completedFuture, resourceManager,
   ...
   */
  public CompletableFuture<Void> getReloadFuture() { return this.reloadFuture; }
  ...
  /**
   * Retrieves a readable name for a shader type. Supports all shader types
   * instead of just vertex and fragment.
   *
  ...
  private record
  ReloadState(Map<ResourceLocation, ProgramDefinition> definitions,
              Map<ResourceLocation, Resource> shaderSources) {}
}
