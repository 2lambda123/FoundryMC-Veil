 ...
 * @author Ocelot
 */
public class ShaderModificationManager extends SimplePreparableReloadListener<
    ShaderModificationManager.Preparations> {

  public static final FileToIdConverter MODIFIER_LISTER =
      new FileToIdConverter("pinwheel/shader_modifiers", ".txt");
  private static final Map<String, String> NEXT_STAGES =
      Map.of("vsh", "gsh", "gsh", "fsh");
  ...
  public String applyModifiers(ResourceLocation shaderId, String source,
                               int flags) {
    //        try {
    //            return this.transformer.transform(source, new
    //            VeilJobParameters(this, shaderId, flags));
    //        } catch (TransformationException | ParsingException |
    //        IllegalStateException | IllegalArgumentException e) {
    //            Veil.LOGGER.error("Failed to transform shader: {}", shaderId,
    //            e);
    //        }
    return source;
  }
   ...
   */
  public Collection<ShaderModification>
  getModifiers(ResourceLocation shaderId) {
   ...
   */
  public @Nullable
  ResourceLocation getModifierId(ShaderModification modification) {
    return this.names.get(modification);
  }

  private @Nullable ResourceLocation
  getNextStage(ResourceLocation shader, ResourceProvider resourceProvider) {
    ...
    while (extension != null) {
      ...
      ResourceLocation nextShader = new ResourceLocation(
          shader.getNamespace(),
          shader.getPath().substring(0, shader.getPath().length() - 3) +
              extension);
  ...
  @Override
  protected @NotNull
  Preparations prepare(@NotNull ResourceManager resourceManager,
    ...
    for (Map.Entry<ResourceLocation, Resource> entry :
         MODIFIER_LISTER.listMatchingResources(resourceManager).entrySet()) {
      ...
      try {
        ...
        if (parts.length < 2) {
          Veil.LOGGER.warn(
              "Ignoring shader modifier {}. Expected format to be located in " +
              "shader_modifiers/domain/shader_path.vsh.txt",
              file);
          continue;
        }
        ...
        try (Reader reader = entry.getValue().openAsReader()) {
          ShaderModification modification = ShaderModification.parse(
              IOUtils.toString(reader), shaderId.getPath().endsWith(".vsh"));
          List<ShaderModification> modifications =
              modifiers.computeIfAbsent(shaderId, name -> new LinkedList<>());
          ...
          if (modifications.size() != 1 ||
              !(modifications.get(0) instanceof ReplaceShaderModification)) {
    ...
    for (Map.Entry<ResourceLocation, List<ShaderModification>> entry :
         new HashMap<>(modifiers).entrySet()) {
      ...
      for (ShaderModification modification : entry.getValue()) {
        ...
        InputShaderModification input = new InputShaderModification(
            simpleMod.priority(),
            OUT_PATTERN
                .matcher(simpleMod.fillPlaceholders(simpleMod.getOutput()))
                .replaceAll("in "));
        modifiers.computeIfAbsent(nextStage, unused -> new LinkedList<>())
            .add(input);
        names.put(input, names.get(simpleMod));
      }
    }
    modifiers.values().forEach(
        modifications
        -> modifications.sort(
            Comparator.comparingInt(ShaderModification::priority)
  ...
  @Override
  protected void apply(@NotNull Preparations preparations,
  ...
  @ApiStatus.Internal
  public record
  Preparations(Map<ResourceLocation, List<ShaderModification>> shaders,
